...
export const Dock = ({ items, activeItems }: DockProps) => {
    ...
    useEffect(() => {
        const currentDockRef = dockRef.current; // Capture the ref value

        const handleMouseMove = (e: MouseEvent) => {
            if (currentDockRef) {
                const dockRect = currentDockRef.getBoundingClientRect();
        ...
        currentDockRef?.addEventListener("mouseleave", handleMouseLeave);

        return () => {
            document.removeEventListener("mousemove", handleMouseMove);
            currentDockRef?.removeEventListener("mouseleave", handleMouseLeave); // Use the captured variable
        };
    }, []);
                                ...
                                >
                                    ...
                                    <div className="relative flex flex-col items-center">
                                        ...
                                        <button
                                            ...
                                            className={cn("w-14 h-14 rounded-[16px] flex items-center justify-center transition-all duration-200 relative", "active:scale-90", "hover:brightness-110")}
                                            style={{
